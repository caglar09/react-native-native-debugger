...
function parseArgs(argv) {
  const args = { command: 'doctor', root: process.cwd(), strict: false, json: false, platform: 'android', host: '127.0.0.1', port: 9876, noOpen: false, iosDevice: false, connect: null, mcpTimeout: 5000 };
  ...
  for (let i = 0; i < rest.length; i++) {
    ...
    else if (value === '--connect' && rest[i + 1]) args.connect = rest[++i];
    else if (value === '--mcp-timeout' && rest[i + 1]) args.mcpTimeout = Number(rest[++i]);
  ...
  if (!Number.isFinite(args.mcpTimeout) || args.mcpTimeout < 500 || args.mcpTimeout > 60000) throw new Error(`Invalid MCP timeout: ${args.mcpTimeout}`);
  return args;
}
...
(async function main() {
  try {
    ...
    switch (args.command) {
      ...
      case 'mcp': await runMcp(args); break;
