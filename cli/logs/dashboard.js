...
const MAX_BUFFER=5000;let paused=false,buffer=[],queue=[],flushTimer=null;const selectedIds=new Set();
...
function filteredAll(){return buffer.filter(matches)}function filtered(){return filteredAll().slice(0,Number(limitEl.value||100))}
...
function refreshFacets(){setOptions(levelEl,new Set(buffer.map(levelName)),'All levels');setOptions(sourceEl,new Set(buffer.map(sourceName)),'All packages');setOptions(serviceEl,new Set(buffer.map(serviceName)),'All services')}
function detailObject(e){const out={...e};delete out.id;return out}
function pruneSelection(){const ids=new Set(buffer.map(e=>e.id));for(const id of selectedIds)if(!ids.has(id))selectedIds.delete(id)}
...
function exportSelected(){if(!paused)return;const data=buffer.filter(e=>e.id&&selectedIds.has(e.id));download(data,'json','selected')}
...
for(const el of [searchEl,levelEl,sourceEl,serviceEl,limitEl])el.addEventListener(el===searchEl?'input':'change',render);speedEl.onchange=()=>{if(!paused){if(flushTimer){clearTimeout(flushTimer);flushTimer=null}scheduleRender()}};
pauseBtn.onclick=()=>{paused=!paused;statusEl.textContent=paused?'● PAUSED':'● LIVE';statusEl.className=paused?'paused':'live';pauseBtn.textContent=paused?'Resume':'Pause';if(!paused)scheduleRender();updateControls()};
document.querySelector('#clear').onclick=()=>{buffer=[];queue=[];selectedIds.clear();if(flushTimer){clearTimeout(flushTimer);flushTimer=null}refreshFacets();render()};exportJsonBtn.onclick=()=>exportFiltered('json');exportNdjsonBtn.onclick=()=>exportFiltered('ndjson');exportSelectedBtn.onclick=exportSelected;
